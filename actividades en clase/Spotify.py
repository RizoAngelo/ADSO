...
def agregarcancionalartista(Spotify):
    ...
    if Artista not in Spotify:
        ...
        if a == "si":
            Artista=input('Ingrese nombre del artista: ')
            Spotify.update({Artista:{}})
            return agregarcancionalartista(Spotify)#repr

        elif a == 'no':
            return ("regreso a la lista"), Spotify
        elif a == "buscar":
...
def eliminarunartista(Spotify):
    ...
    for G in sorted(Spotify.keys()):
        if eliminar==G:
            del Spotify[G]
            return("El (La) artista",eliminar,"fue eliminada con exito de Spotify")
        return None
    return("esta cancion no se encuentra en la Spotify")
def mayor(Spotify):
    a=int
    mayor_valor=0
